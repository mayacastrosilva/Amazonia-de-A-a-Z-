...
if($categoria->slug)
{
    $totalPerPage = 8;
    $siteId = WP_SITE_ID;
    $query = "/sumauma/posts?categories={$categoria->slug}&limit={$totalPerPage}&post_status=publish,private&site_blog_id={$siteId}";
    $page = 1;
    ...
    $results = get_posts_category_api($query);
    ...
    $mensagemResultado = ($results) ? "<h1>Resultado de busca para Categoria: <b class=\"page-title\">$categoria->name</b></h1><p>{$results->totalFiltered} resultado(s) encontrado(s)</p>" : "<h1>Nenhum resultado encontrado para a tag: <b class=\"page-title\">$categoria->name</b></h1>";
